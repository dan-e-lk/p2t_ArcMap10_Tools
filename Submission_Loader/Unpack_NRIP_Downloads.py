...
#
# Note that this tool only works best on AR and AWS, final/draft FMP and FMP contingency plan. It's not intended for intermediate products such as checkpoint 1 submission.
#
#
# Assumptions:
# The input folder (or zipfile) contains at least one folder with the following naming convention: "NRIP_Thu Feb 18 2021_SUB-FM-110-2012-AWS-464.zip".
#
# Limitations:
# The tool was intended to work on new AWS, AR, and FMP submissions that follows the post-2017 tech spec. 
# 
# 
# 
#
...
# from Loader.functions import print2, rand_alphanum_gen # delete this line later
from Loader.functions import rand_alphanum_gen

def main(NRIPDownloadzipfile,outputFMPfolder,openfolder):
	"""
	NRIPDownloadzipfile is the file location of the zipfile downloaded off the FI Portal (can be a zipfile or a non-zipfile)
	...
	this tool will unpack submission into 'C:\\user\\_FMP_LOCAL\\Hearst\\AWS\\2018'
	"""

	# start logging
	logfile = os.path.join(outputFMPfolder, 'log_sem.txt') # !!!! This has to be edited later !!!
	log_level = 'DEBUG' if debug else 'INFO'
	l = arclog.Log(log_level)
	l.logger('### ## #  Launching Unpack NRIP Downloads program  # ## ###')
...
if __name__ == '__main__':
	...
	# NRIPDownloadzipfile = r'C:\FIPDownload\download_cart_2018-10-30ar.zip'
	...
	# main(NRIPDownloadzipfile,outputFMPfolder)



	NRIPDownloadzipfile = arcpy.GetParameterAsText(0) # must be an existing zipfile
	...
	main(NRIPDownloadzipfile,outputFMPfolder,openfolder)