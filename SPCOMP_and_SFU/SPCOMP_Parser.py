...
def spParse(inputfc,outputfc,spfield,create_all_spcfields = True):
    ...
    if not create_all_spcfields:
        ...
        full_spcomp = []
        with arcpy.da.SearchCursor(outputfc, [spfield]) as cursor:
            for row in cursor:
    ...
    ##completeFldList.append(check_field)
    ##if check_field not in existingFields:
    ##    arcpy.AddMessage("Creating a new field: %s"%check_field)
    ##    arcpy.AddField_management(in_table = outputfc, field_name = check_field, field_type = "TEXT", field_length = "20")
