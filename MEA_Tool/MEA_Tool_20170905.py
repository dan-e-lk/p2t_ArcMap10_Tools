...
olt = str(arcpy.GetParameterAsText(3)) # OLT moose carrying capacity shapefile or feature class
bmi = str(arcpy.GetParameterAsText(4)) # BMI or PCI that has "OWNER" field
arCC = str(arcpy.GetParameterAsText(5)) # Don't have to be AR. Polygons of previous harvests (preferablly clear cut) will do.
arSH = str(arcpy.GetParameterAsText(6))
arSE = str(arcpy.GetParameterAsText(7))
drvRoad = str(arcpy.GetParameterAsText(8)) # will be clipped, buffered by 500m and 1km
winterCov = str(arcpy.GetParameterAsText(9)) # also optional. This input must be the output of the Moose Winter Cover Delineator tool.
...
if fc not in fcList:
    ...
    arcpy.FeatureClassToFeatureClass_conversion(in_features=r"N:\NDD\GDDS-Internal-MNRF.gdb\forest_management_unit", out_path= workspace, out_name=fc, where_clause="FMU_NAME = '" + fmu + " Forest'")
else:
    arcpy.AddMessage("%s already exists."%fc)
...
# MAFAs (clip to fmu + 10k buffer, then select by habitat rank)
mafafc = fmu.replace(' ','_') + '_MAFA'
fc = mafafc
if fc not in fcList:
    arcpy.AddMessage("Creating a new fc: %s..."%fc)
    arcpy.Clip_analysis(in_features=r"N:\NDD\GDDS-Internal-MNRF.gdb\AQUATIC_FEEDING_AREA", clip_features = mufc10k, out_feature_class = fc)
else:
    arcpy.AddMessage("%s already exists."%fc)


fc = fmu.replace(' ','_') + '_MAFA34'
if fc not in fcList:
    arcpy.AddMessage("Creating a new fc: %s..."%fc)
    arcpy.FeatureClassToFeatureClass_conversion(in_features = mafafc, out_path= workspace, out_name=fc, where_clause="HABITAT_RANK_CODE in('3','4')")
else:
    arcpy.AddMessage("%s already exists."%fc)
unionList.append(fc)


fc = fmu.replace(' ','_') + '_MAFA12'
if fc not in fcList:
    arcpy.AddMessage("Creating a new fc: %s..."%fc)
    arcpy.FeatureClassToFeatureClass_conversion(in_features = mafafc, out_path= workspace, out_name=fc, where_clause="HABITAT_RANK_CODE in('1','2')")
else:
    arcpy.AddMessage("%s already exists."%fc)
unionList.append(fc)


# Wintering Area (clip to fmu + 10k buffer, then select by class subtytpe)

wafc = fmu.replace(' ','_') + '_WintArea'
fc = wafc
if fc not in fcList:
    arcpy.AddMessage("Creating a new fc: %s..."%fc)
    arcpy.Clip_analysis(in_features=r"N:\NDD\GDDS-Internal-MNRF.gdb\WINTERING_AREA", clip_features = mufc10k, out_feature_class = fc)
else:
    arcpy.AddMessage("%s already exists."%fc)

fc = fmu.replace(' ','_') + '_WintArea_Moose'
whereClause = "CLASS_SUBTYPE in('Moose Early Wintering Area', 'Moose Late Wintering Area')"
if fc not in fcList:
    arcpy.AddMessage("Creating a new fc: %s..."%fc)
    arcpy.FeatureClassToFeatureClass_conversion(in_features = wafc, out_path= workspace, out_name=fc, where_clause= whereClause)
else:
    arcpy.AddMessage("%s already exists."%fc)
unionList.append(fc)

fc = fmu.replace(' ','_') + '_WintArea_Deer'
whereClause = "CLASS_SUBTYPE in('Deer Wintering Area (Stratum 2)', 'Deer Yard (Stratum 1)')"
if fc not in fcList:
    arcpy.AddMessage("Creating a new fc: %s..."%fc)
    arcpy.FeatureClassToFeatureClass_conversion(in_features = wafc, out_path= workspace, out_name=fc, where_clause= whereClause)
else:
...
if fc not in fcList:
    arcpy.AddMessage("Creating a new fc: %s..."%fc)
    arcpy.Clip_analysis(in_features=r"N:\NDD\GDDS-Internal-MNRF.gdb\CLUPA_PROVINCIAL", clip_features = mufc10k, out_feature_class = fc)
else:
    arcpy.AddMessage("%s already exists."%fc)
...
if fc not in fcList:
    arcpy.AddMessage("Creating a new fc: %s..."%fc)
    arcpy.Clip_analysis(in_features=r"N:\NDD\GDDS-Internal-MNRF.gdb\OHN_WATERBODY", clip_features = meafc, out_feature_class = fc)
else:
    arcpy.AddMessage("%s already exists."%fc)
...
if fc not in fcList:
    try:
        ...
        pass
    arcpy.AddMessage("Creating a new fc: %s..."%fc)
    arcpy.Clip_analysis(in_features=r"N:\NDD\GDDS-Internal-MNRF.gdb\WETLAND", clip_features = meafc, out_feature_class = "Wetland_temp")
