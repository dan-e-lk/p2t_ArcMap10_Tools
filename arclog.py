	...
	def logger(self, msg,level='INFO'):
		...
		if current_level >= self.cutoff_level:
			...
			if self.iprint:
				print(new_msg)
				if current_level == 50:
					arcpy.AddError(new_msg)
				elif current_level >= 30:
					arcpy.AddWarning(new_msg)
				else:
					arcpy.AddMessage(new_msg)

			if self.ireturn: return new_msg

	def msg_editor(self, msg, level):
		if self.disp_time_n_lvl:
			datetime_now = datetime.datetime.now().strftime('%H:%M:%S.%f')[:-3] # eg. '2018.09.07 14:41:13.659'
