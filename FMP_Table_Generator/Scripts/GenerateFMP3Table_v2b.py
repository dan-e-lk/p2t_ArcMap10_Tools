...
import arcpy
import os, datetime, webbrowser, inspect, traceback
...
dev_field = arcpy.GetParameterAsText(3) # NEXTSTG or DEVSTAGE
...
htmlstring = htmlstring.replace("*ForestUnitField", fu_field)
htmlstring = htmlstring.replace("*SoM", dev_field)
...
for fu, value in summary_dict.items():
    ...
    for i, agegroup in enumerate(value):
        ...
        # special cases where there are more than one stage of management
        SoM_dict = {}
        if len(set_of_SoM) > 1:
            for SoM in set_of_SoM:
                area = 0
                with arcpy.da.SearchCursor(inputbmi,f,prod_for_avail_sql_full) as cursor:
                    for row in cursor:
                        if row[f.index(dev_field)] == SoM and row[f.index('AGE')] >= age_from and row[f.index('AGE')] <= age_to:
                            area += row[f.index(AreaField)]
                SoM_dict[SoM] = area/10000
        summary_dict[fu][i].append(SoM_dict)
...
if debug: ftf.logger("%s"%summary_dict)

# Creating a CSV file out of the summary dict
try:
    import ToCsv
    csvfilename = ToCsv.main(summary_dict, dev_field, reportfile)
    ftf.logger('CSV file created.')
except:
    ftf.logger("Failed to create a csv file")
    ftf.logger(traceback.format_exc())
...
for fu, value in sorted(summary_dict.items()):
    ...
    replace_dict = {'*NumOfAgeGroups':str(len(age_increment)),
                    ...
                    '*SoM':           str(value[0][3]),
                    '*Avail':         ftf.avail_editor(value[0][4], value[0][5])}
    ...
    if len(age_increment) > 2:
        for i, agegroup in enumerate(age_increment[1:-1]):
            replace_dict = {'*AgeGroup':      '%s - %s'%(str(value[i+1][0][0]),str(value[i+1][0][1])),
                            ...
                            '*SoM':           str(value[i+1][3]),
                            '*Avail':         ftf.avail_editor(value[i+1][4],value[i+1][5])}
    ...
    replace_dict = {'*AgeGroup':      '%s - %s'%(str(value[-1][0][0]),str(value[-1][0][1])),
                    ...
                    '*SoM':           str(value[-1][3]),
                    '*Avail':         ftf.avail_editor(value[-1][4], value[-1][5])}
